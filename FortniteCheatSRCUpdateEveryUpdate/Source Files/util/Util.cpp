...
namespace Util {
	...
	BOOLEAN Initialize() {
		...
		// xxx????x????xxx
	        ...
	        auto addr = FindPattern("\x48\x00\x00\x00\x00\x00\x00\x48\x98\x4C\x8B\x04\xD1\x48\x8D\x0C\x40\x49\x8D\x04\xC8\xEB\x02", "xx??????xxxxxxxxxxxxxxxx");
		if (!addr) {
		...
		addr = FindPattern("\x48\x89\x5C\x24\x18\x48\x89\x74\x24\x20\x55\x57\x41\x56\x48\x8D\xAC\x24\xB0\xF8", "xxxxxxxxxxxxxxxxxxxx");
		if (!addr) {
		...
		addr = FindPattern("\x48\x89\x5c\x24\x00\x55\x56\x57\x48\x8d\x6c\x24\x00\x48\x81\xec\x00\x00\x00\x00\x48\x8b\x05\x00\x00\x00\x00\x48\x33\xc4\x48\x89\x45\x37\x49\x8b\xf8", "xxxx?xxxxxxx?xxx????xxx????xxxxxxxxxx");
		if (!addr) {
